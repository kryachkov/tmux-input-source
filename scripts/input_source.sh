...
print_input_source() {
  defaults read ~/Library/Preferences/com.apple.HIToolbox.plist | grep --color=never AppleCurrentKeyboardLayoutInputSourceID | awk '{print $3}' | sed "s/\"//g" | sed 's/;//' | awk -F. '{print $4}'
}
